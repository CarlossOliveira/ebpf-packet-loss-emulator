...
BPF_UTILS_DIR := $(PROJECT_ROOT)/src/bpf/utils

OUTPUT_DIR := $(PROJECT_ROOT)/build
BIN_DIR := $(OUTPUT_DIR)/bin
BPF_MODULES_DIR := $(OUTPUT_DIR)/bpf/modules

APP_BIN := $(BIN_DIR)/bpf-packet-loss-emulator

BPF_SRCS := $(wildcard $(BPF_DIR)/*.bpf.c)
BPF_OBJS := $(patsubst $(BPF_DIR)/%.bpf.c,$(BPF_MODULES_DIR)/%.bpf.o,$(BPF_SRCS))
...
app:
	...
	@mkdir -p $(BIN_DIR)
	$(CC) $(CFLAGS) -DAPP -DPROJECT_ROOT=\"$(PROJECT_ROOT)\" -DBPF_MODULES_DIR=\"$(BPF_MODULES_DIR)\" -I$(CORE_INCLUDE_DIR) -I$(UTILS_INCLUDE_DIR) $(APP_SRCS) -o $(APP_BIN) $(LIBS)
	@echo "Loader application compiled successfully."

bpf: $(BPF_OBJS)
	@echo "eBPF modules compiled successfully."

$(BPF_MODULES_DIR)/%.bpf.o: $(BPF_DIR)/%.bpf.c
	@echo "Compiling BPF module $<..."
	@mkdir -p $(BPF_MODULES_DIR)
	$(BPF_CC) -DBPF -DPROJECT_ROOT=\"$(PROJECT_ROOT)\" -DBPF_MODULES_DIR=\"$(BPF_MODULES_DIR)\" -I$(BPF_DIR)/helpers.bpf.h -I$(BPF_UTILS_DIR) -I$(CORE_INCLUDE_DIR) -I$(UTILS_INCLUDE_DIR) $(BPF_CFLAGS) -c $< -o $@
